...

{
    ...
    */
...

    {
        ...
        if(Session::has('teamId') && Session::has('teamType'))

        {
            $team_id = Session::get('teamId');
            $team_type = Session::get('teamType');

            if($team_type == "Purchasing"){

                $team = BuyerTeam::where("team_id","=",$team_id)->first();
                if(!empty($team)){
                    $buyerTeamUser = new BuyerTeamUser();
                    $buyerTeamUser->team_id = $team->id;
                    $buyerTeamUser->user_id = $user->id;
                ...
                $team = SupplierTeam::where("supplier_team_id","=",$team_id)->first();
                if(!empty($team)){
                    $supplierTeamUser = new SupplierTeamUser();
                    $supplierTeamUser->supplier_team_id = $team->id;
                    $supplierTeamUser->user_id = $user->id;
