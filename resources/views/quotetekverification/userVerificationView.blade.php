        ...
        <div class="portlet box red">
            ...
            <div class="portlet-body">
                ...
                <div class="row">
                    <div class="col-md-12">
                        @if($userVerififcation != '')
                            @if($userVerififcation->status == 1)
                                    Your Verification Approved.
                            @elseif($userVerififcation->status == 2)
                                Your Verification Disapproved. Please Send again 
                                <a href="{{ URL::to('quotetek/user/vrification') }}" class="btn btn-circle btn-sm"><i class="fa fa-plus"></i>  Add New</a>
                            @else
                                Your verification request is received and pending, we will contact you if any further information is needed.
                            @endif
                        @else
                            Please Send Verification request <a href="{{ URL::to('quotetek/user/vrification') }}" class="btn btn-circle btn-sm"><i class="fa fa-plus"></i>  Add New</a>
                        @endif
                    </div>
                </div>
            </div>
